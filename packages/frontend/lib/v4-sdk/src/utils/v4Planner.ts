...
export class V4Planner {
  ...
  addSettle(currency: Currency, payerIsUser: boolean, amount?: bigint): V4Planner {
    this.addAction(Actions.SETTLE, [currencyAddress(currency), amount ?? BigInt(FULL_DELTA_AMOUNT), payerIsUser])
    return this
  }

  addTake(currency: Currency, recipient: string, amount?: bigint): V4Planner {
    const takeAmount = amount ?? BigInt(FULL_DELTA_AMOUNT)
  ...
  finalize(): string {
    ...
    return abiCoder.encode(['bytes', 'bytes[]'], [this.actions, this.params])
  }
}
